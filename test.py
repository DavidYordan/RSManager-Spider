...
class Test(object):
    ...
    async def main(self):
        Globals.logger.info('Starting the test.', self.user)
        try:
            await self.api.create_sessions()
            Globals.logger.info('Session created successfully.', self.user)
            user_info = await self.api.user(username="tiffany.lee14").info()
            Globals.logger.info(user_info, self.user)
            async for video in self.api.user(username="tiffany.lee14").videos():
                video_info = video.as_dict
                Globals.logger.info(video_info, self.user)
        except Exception as e:
            Globals.logger.error(f"Error occurred: {e}", self.user)
